...
const CategoryDetailPage = () => {
  ...
  useEffect(() => {
    let isMounted = true;
    if (categorySlug) {
      fetchVehicleCategoryBySlug(categorySlug).then((data) => {
        if (isMounted) {
  ...
  const galleryImages = useMemo(() => {
    ...
    if (category.image) set.add(formatImageUrl(category.image));
    (category.gallery || []).forEach((img) => set.add(formatImageUrl(img)));
    ...
    <main className="min-h-screen bg-[#121212] text-white">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        <motion.div 
          ...
          style={{ backgroundImage: `url(${formatImageUrl(category.image)})`, transform: 'translateZ(0)' }}
        >
